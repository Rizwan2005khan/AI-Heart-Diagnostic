...
![Project Roadmap](ai_diagnostics/readme_images/Project_ReadMap.svg)
...
![Dataset Profile](ai_diagnostics/readme_images/dataset_profile.svg)

### The Acquisition Flow — `data_loader.py`
This script handles the downloading and initial loading of the data.

![Data Acquisition Flow](ai_diagnostics/readme_images/data_acquisition_flow.svg)
...
![Data Cleaning Problems](ai_diagnostics/readme_images/data_cleaning_problems.svg)
...
*   **Problem 2: Missing Value Strategy**: We use `df.dropna()`. Losing 6 out of 303 rows is acceptable for this dataset.
    ![Missing Value Strategies](ai_diagnostics/readme_images/missing_value_strategies.svg)
*   **Problem 3: Target Encoding**: Converting severity levels to a clean yes/no binary target.
    ![Lambda Encoding](ai_diagnostics/readme_images/lambda_target_encoding.svg)
...
> ![Skip Cleaning Consequences](ai_diagnostics/readme_images/skip_cleaning_consequences.svg)
...
![EDA Motivation](ai_diagnostics/readme_images/eda_motivation.svg)
...
2.  **Correlation Heatmap**: Identifying which features (like `cp`, `exang`, `oldpeak`) correlate most with disease.
    ![Correlation Scale](ai_diagnostics/readme_images/correlation_scale_explainer.svg)
...
4.  **Chest Pain Type vs Disease**: Revealing that asymptomatic patients (`cp=3`) often have the highest disease counts.
    ![Chest Pain Insight](ai_diagnostics/readme_images/chest_pain_insight.svg)
...
![What Training Means](ai_diagnostics/readme_images/what_training_means.svg)
...
> ![No Free Lunch](ai_diagnostics/readme_images/no_free_lunch.svg)

### Algorithms Evaluated:
*   **Logistic Regression**: Simple, interpretable baseline.
    ![Logistic Regression](ai_diagnostics/readme_images/logistic_regression_intuition.svg)
*   **KNN (K-Nearest Neighbors)**: Predicts based on the most similar past patients.
    ![KNN Intuition](ai_diagnostics/readme_images/knn_intuition.svg)
*   **SVM (Support Vector Machine)**: Finds the "maximum margin" boundary. **Our Winner!**
    ![SVM Margin](ai_diagnostics/readme_images/svm_maximum_margin.svg)
*   **Random Forest & XGBoost**: Advanced ensemble methods using bagging and boosting.
    ![Bagging vs Boosting](ai_diagnostics/readme_images/bagging_vs_boosting_diagram.svg)

### Why Recall Matters
In medical diagnostics, we prioritize **Recall** to minimize False Negatives (missing a sick patient).
![Recall vs Precision](ai_diagnostics/readme_images/precision_recall_tradeoff.svg)
...
![Streamlit Concept](ai_diagnostics/readme_images/streamlit_concept.svg)
...
*   **Intelligent Caching**: Uses `@st.cache_resource` for blazing-fast model loading.
    ![Caching Flow](ai_diagnostics/readme_images/cache_resource_flow.svg)
...
---
*Created with ❤️ for AI Diagnostic Research.*